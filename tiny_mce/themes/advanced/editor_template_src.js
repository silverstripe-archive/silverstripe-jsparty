...
function TinyMCE_advanced_getEditorTemplate(settings, editorId)
{
	...
	switch(layoutManager) {
		case "SimpleLayout" : //the default TinyMCE Layout (for backwards compatibility)...
			...
			if(toolbarLocation != "manually_placed") {
			...
			{
				var bod = tinyMCE.getParam("theme_advanced_toolbar_parent");
				if(bod) bod = document.getElementById(bod);
				else bod = document.body;
				...
				elm.innerHTML = '<table width="100%" border="0" align="' + toolbarAlign + '"><tr><td align="' + toolbarAlign + '">'+toolbarHTML+'</td></tr></table>';
				bod.appendChild (elm);
				...
				editor.toolbarElement = elm;
				editor.toolbarElement.editor = editor;
			...
			} else if(toolbarLocation == "manually_placed") {
				tinyMCE.getInstanceById(editorId).toolbarElement = function() { return Toolbar.instance(); }
...
function TinyMCE_advanced_handleNodeChange(editor_id, node, undo_index, undo_levels, visual_aid, any_selection, setup_content) {
	...
	// Reset old states
	tinyMCE.switchClassSticky(toolbar_id + '_justifyleft', 'mceButtonNormal');
	tinyMCE.switchClassSticky(toolbar_id + '_justifyright', 'mceButtonNormal');
	tinyMCE.switchClassSticky(toolbar_id + '_justifycenter', 'mceButtonNormal');
	tinyMCE.switchClassSticky(toolbar_id + '_justifyfull', 'mceButtonNormal');
	tinyMCE.switchClassSticky(toolbar_id + '_bold', 'mceButtonNormal');
	tinyMCE.switchClassSticky(toolbar_id + '_italic', 'mceButtonNormal');
	tinyMCE.switchClassSticky(toolbar_id + '_underline', 'mceButtonNormal');
	tinyMCE.switchClassSticky(toolbar_id + '_strikethrough', 'mceButtonNormal');
	tinyMCE.switchClassSticky(toolbar_id + '_bullist', 'mceButtonNormal');
	tinyMCE.switchClassSticky(toolbar_id + '_numlist', 'mceButtonNormal');
	tinyMCE.switchClassSticky(toolbar_id + '_sub', 'mceButtonNormal');
	tinyMCE.switchClassSticky(toolbar_id + '_sup', 'mceButtonNormal');
	tinyMCE.switchClassSticky(toolbar_id + '_anchor', 'mceButtonNormal');
	tinyMCE.switchClassSticky(toolbar_id + '_link', 'mceButtonDisabled', true);
	tinyMCE.switchClassSticky(toolbar_id + '_unlink', 'mceButtonDisabled', true);
	tinyMCE.switchClassSticky(toolbar_id + '_outdent', 'mceButtonDisabled', true);
	tinyMCE.switchClassSticky(toolbar_id + '_image', 'mceButtonNormal');
	tinyMCE.switchClassSticky(toolbar_id + '_hr', 'mceButtonNormal');

	if (node.nodeName == "A" && tinyMCE.getAttrib(node, "class").indexOf('mceItemAnchor') != -1)
		tinyMCE.switchClassSticky(toolbar_id + '_anchor', 'mceButtonSelected');
	...
	{
		tinyMCE.switchClassSticky(toolbar_id + '_link', anchorLink ? 'mceButtonSelected' : 'mceButtonNormal', false);
		tinyMCE.switchClassSticky(toolbar_id + '_unlink', anchorLink ? 'mceButtonSelected' : 'mceButtonNormal', false);
	}

	// Handle visual aid
	tinyMCE.switchClassSticky(toolbar_id + '_visualaid', visual_aid ? 'mceButtonSelected' : 'mceButtonNormal', false);

	if (undo_levels != -1)
	{
		tinyMCE.switchClassSticky(toolbar_id + '_undo', 'mceButtonDisabled', true);
		tinyMCE.switchClassSticky(toolbar_id + '_redo', 'mceButtonDisabled', true);
	}

	// Within li, blockquote
	if (tinyMCE.getParentElement(node, "li,blockquote"))
	{
		tinyMCE.switchClassSticky(toolbar_id + '_outdent', 'mceButtonNormal', false);
	}

	// Has redo levels
	if (undo_index != -1 && (undo_index < undo_levels-1 && undo_levels > 0))
	{
		tinyMCE.switchClassSticky(toolbar_id + '_redo', 'mceButtonNormal', false);
	}

	// Has undo levels
	if (undo_index != -1 && (undo_index > 0 && undo_levels > 0))
	{
		tinyMCE.switchClassSticky(toolbar_id + '_undo', 'mceButtonNormal', false);
	}

	// Select class in select box
	var selectElm = document.getElementById(toolbar_id + "_styleSelect");
	
	if (selectElm)
	{
		TinyMCE_advanced_setupCSSClasses(toolbar_id);
	...
	// Select formatblock
	var selectElm = document.getElementById(toolbar_id + "_formatSelect");
	if (selectElm) {
	...
	// Select fontselect
	var selectElm = document.getElementById(toolbar_id + "_fontNameSelect");
	if (selectElm) {
	...
	// Select fontsize
	var selectElm = document.getElementById(toolbar_id + "_fontSizeSelect");
	if (selectElm) {
	...
	do {
		...
		switch (alignNode.getAttribute('align').toLowerCase()) {
			case "left":
				tinyMCE.switchClassSticky(toolbar_id + '_justifyleft', 'mceButtonSelected');
				breakOut = true;
			break;

			case "right":
				tinyMCE.switchClassSticky(toolbar_id + '_justifyright', 'mceButtonSelected');
				breakOut = true;
			break;

			case "middle":
			case "center":
				tinyMCE.switchClassSticky(toolbar_id + '_justifycenter', 'mceButtonSelected');
				breakOut = true;
			break;

			case "justify":
				tinyMCE.switchClassSticky(toolbar_id + '_justifyfull', 'mceButtonSelected');
				breakOut = true;
			break;
		}
	...
	if (!setup_content) {
		...
		for (var i=0; i<ar.length; i+=2) {
			if (inst.queryCommandState(ar[i]))
				tinyMCE.switchClassSticky(toolbar_id + ar[i+1], 'mceButtonSelected');
		}

		if (inst.queryCommandState("Underline") && (node.parentNode == null || node.parentNode.nodeName != "A")) {
			tinyMCE.switchClassSticky(toolbar_id + '_underline', 'mceButtonSelected');
		}
	}
...
/*			case "B":
			case "STRONG":
				tinyMCE.switchClassSticky(toolbar_id + '_bold', 'mceButtonSelected');
			break;

			case "I":
			case "EM":
				tinyMCE.switchClassSticky(toolbar_id + '_italic', 'mceButtonSelected');
			break;
			...
			case "STRIKE":
				tinyMCE.switchClassSticky(toolbar_id + '_strikethrough', 'mceButtonSelected');
			break;*/

			case "UL":
				tinyMCE.switchClassSticky(toolbar_id + '_bullist', 'mceButtonSelected');
			break;

			case "OL":
				tinyMCE.switchClassSticky(toolbar_id + '_numlist', 'mceButtonSelected');
			break;

			case "HR":
				 tinyMCE.switchClassSticky(toolbar_id + '_hr', 'mceButtonSelected');
			break;

			case "IMG":
			if (getAttrib(node, 'name').indexOf('mce_') != 0) {
				tinyMCE.switchClassSticky(toolbar_id + '_image', 'mceButtonSelected');
			}
			break;
		}
...
function TinyMCE_advanced_setupCSSClasses(toolbar_id) {
	...
	if (selectElm && selectElm.getAttribute('cssImported') != 'true') {
		var csses = tinyMCE.getCSSClasses(toolbar_id);
		if (csses && selectElm)	{
