...
function TinyMCE_contextmenu_showContextMenu(e, inst) {
	...
	if(tinyMCE.isMSIE) {
		...
		x = pos.absLeft + (e.pageX - inst.getBody().scrollLeft);
		y = pos.absTop + (e.pageY - inst.getBody().scrollTop);
		
		// SilverStripe modification: Added 29/1/07 for trac #185.
		// Add the scroll-top of the editor's container elements
		var scrollContainer = inst.iframeElement.parentNode;
		while(scrollContainer.tagName.toLowerCase() != 'body') {
...
ContextMenu.prototype.show = function(x, y) {
	...
	// SilverStripe modification: Fix context menu so that it can't be shown off the bottom of the screen
	if( (y + height) > document.body.offsetHeight ) y = document.body.offsetHeight - height;
