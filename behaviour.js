...
var Behaviour = {
	...
	addLoader : function(func){
		...
		// Preserve the old window.onload
		if(window.setupForBehaviour) {
		} else {
			window.setupForBehaviour = true;
			if(window.onload) this.loaders.push(window.onload);
			// Set up runLoaders as the onload function - it will call everything in this.loaders
			window.onload = function() { Behaviour.runLoaders(); }
		}
